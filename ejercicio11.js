...
const movies = [
  ...
  function averageMovieDuration(movies) {
    let duracionPeliculas = 0;

    for(let i = 0; i < movies.length; i++){
        const movie = movies[i];
        duracionPeliculas += movie.duration;
    }

    return duracionPeliculas / movies.length;
  }

  console.log("La duración promedio de las películas es " + averageMovieDuration(movies) + " minutos");
